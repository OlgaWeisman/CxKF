...
from Pipleline_post_KF_CP import Pipeline_post_KF
...
for index in range(0, len(r2)):
    ...
    ####### KalmanNet training ###################
    ##############################################
    KF_Pipeline = Pipeline_KF(strTime, "KNet", "KNet_" + dataFileName[index])
    KF_Pipeline.setssModel(sys_model)
    KalmanNet_model = KalmanNetNN()
    KalmanNet_model.Build(sys_model)
    KF_Pipeline.setModel(KalmanNet_model)
    KF_Pipeline.setTrainingParams(n_Epochs=500, n_Batch=30, learningRate=1E-3, weightDecay=1E-5)
    ...
    ############### Quantiles training ############
    ###############################################
    Quantile_Pipeline = Pipeline_post_KF(strTime, "KNet", "KNet_" + dataFileName[index])
    Quantile_Pipeline.setssModel(sys_model)
    Quantile_model = QuantileNN(new_head_in_shape=2)
    Quantile_Pipeline.setModel(Quantile_model)
    Quantile_Pipeline.setTrainingParams(n_Epochs=500, n_Batch=30, learningRate=1E-3, weightDecay=1E-5)
