...
from Pipeline_EKF_BNN_withCov import Pipeline_EKF

from Extended_BNN_withCov_KalmanNet_nn import KalmanNetNN
...
PFandUKF_test = True ##ause filterpy and pyparticleest are not compatible with GPU
...
dataFileName = ['data_lor_v20_rq020_T2000.pt', 'data_lor_v20_rq1030_T2000.pt', 'data_lor_v20_rq2040_T2000.pt',
                'data_lor_v20_rq3050_T2000.pt', 'data_lor_v20_rq4060_T2000.pt']  # for T=2000
...
for index in range(0, len(r)):
    ...
    [train_input_long, train_target_long, cv_input, cv_target, test_input, test_target] = torch.load(
        DatafolderName + dataFileName[index], map_location=dev)
    if chop:
    ...
    # sys_model = SystemModel(f, q[index], h, r[index], T, T, m, n, "Lor")  # arbitary q and r
    # sys_model.InitSequence(m1x_0, m2x_0)
    # print("KNet with full model info")
    # modelFolder = 'KNet' + '/'
    # KNet_Pipeline = Pipeline_EKF(strTime, "KNet", "KalmanNet")
    # KNet_Pipeline.setssModel(sys_model)
    # KNet_model = KalmanNetNN(device = dev)
    # KNet_model.Build(sys_model)
    # KNet_Pipeline.setModel(KNet_model)
    # KNet_Pipeline.setTrainingParams(n_Epochs=200, n_Batch=10, learningRate=1e-4, weightDecay=1e-4,num_of_mc_iterations = 20)
    #
    ...
    # [KNet_MSE_test_linear_arr, KNet_MSE_test_linear_avg, KNet_MSE_test_dB_avg, KNet_test] = KNet_Pipeline.NNTest(N_T,
    #                                                                                                              test_input,
    #                                                                                                              test_target)
    # KNet_Pipeline.save()
    ...
    sys_model_partialh = SystemModel(f, q[index], hInacc, r[index], T, T_test, m, n,"Lor")
    sys_model_partialh.InitSequence(m1x_0, m2x_0)
    modelFolder = 'KNet' + '/'
