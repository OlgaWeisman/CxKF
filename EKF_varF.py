...
class ExtendedKalmanFilter:
    ...
    def Predict(self):
        ...
        #     self.m1x_prior = (self.f(self.m1x_posterior)).unsqueeze(0)
        # else:
        self.m1x_prior = torch.squeeze(self.f(self.m1x_posterior, t = self.t))

        # Compute the Jacobians
        self.UpdateJacobians(getJacobian(self.m1x_posterior, self.fString, t = self.t), getJacobian(self.m1x_prior, self.hString))
        ...
        # if self.m == 1 and self.n == 1:
        #     self.m1y = (self.h(self.m1x_prior)).unsqueeze(0)
        # else:
    ...
    def Innovation(self, y):
        self.dy = torch.tensor([y - self.m1y])
    ...
    def GenerateSequence(self, y, T):
        ...
        if self.m == 1 and self.n == 1:
            self.m1x_posterior = self.m1x_0.unsqueeze(0)
            self.m2x_posterior = self.m2x_0.unsqueeze(0)
        else:
