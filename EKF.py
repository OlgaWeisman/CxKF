...
class ExtendedKalmanFilter:
    ...
    def Innovation(self, y):
        #self.dy =torch.tensor([y - self.m1y])#y - self.m1y
        self.dy = torch.atleast_1d(torch.as_tensor(y)) - self.m1y
    ...
    def GenerateSequence(self, y, T):
        ...
        for t in range(0, T):
            if y.ndim == 2: yt = torch.squeeze(y[:,  t])
            elif y.ndim == 3: yt = torch.squeeze(y[:, :, t])
