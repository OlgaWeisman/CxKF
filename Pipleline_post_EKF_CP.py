...
class Pipeline_post_EKF:
    ...
    def NNTrain(self, KN_model, n_Examples, train_input, train_target, n_CV, cv_input, cv_target):
        self.Beta = 0.1
        self.N_E = n_Examples
        self.N_CV = n_CV
        ...
        for ti in range(0, self.N_Epochs):
            ...
            for j in range(0, self.N_CV):
                y_cv = cv_input[j, :, :]
                KN_model.InitSequence(self.ssModel.m1x_0, self.ssModel.T)

                x_out_cv = torch.empty(self.ssModel.m, self.ssModel.T)
                q_cv = torch.empty(2, self.ssModel.T)
                for t in range(0, self.ssModel.T):
                    x_out_cv[:, t] = KN_model(y_cv[:, t])
                    q_cv[:, t] = self.model(x_out_cv[:, t], KN_model.y_f1_norm.detach(), KN_model.m1x_f3_norm.detach(),  KN_model.m1x_f4_norm.detach())
            ...
            for j in range(0, self.N_B):
                ...
                for t in range(0, self.ssModel.T):
                    x_out_training[:, t]= KN_model(y_training[:, t])
                    q_train[:, t] = self.model(x_out_training[:, t].detach(), KN_model.y_f1_norm.detach(), KN_model.m1x_f3_norm.detach(),  KN_model.m1x_f4_norm.detach())
                # Compute Training Loss
    ...
    def NNCalibrate_Predict(self, KN_model, n_Calib, n_Trails, calib_input, calib_target, alpha):
        ...
        [q_low, q_hi, _] = compute_gaussian_quantiles(x_array_train.cpu(), sigma_array_train.cpu(), alpha)
        for ii in range(0, n_Trails):
            ...
            for j in range(0, num_of_calib):
                ...
                for t in range(0, self.ssModel.T):
                    x_out_test[:, t] = KN_model(y_mdl_tst[:, t])
                    q_test[:, t] = self.model(x_out_test[:, t].detach(), KN_model.y_f1_norm.detach(), KN_model.m1x_f3_norm.detach(),  KN_model.m1x_f4_norm.detach())
