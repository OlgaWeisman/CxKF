...
class KalmanNetNN(torch.nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") #OLGA
        self.quantiles = [0.05, 0.95]
        self.num_quantiles = len(self.quantiles)
        # Define the new head parameters FIRST
        self.new_head_in_shape = 5 # Insert also delta x and delta y
        self.new_head_hidden_size = 64
        self.new_head_dropout = 0.5
        self.new_head_hidden_gru = 8
    ...
    def step_KGain_est(self, y):
        ...
        # KGain Net Input
        if y_f1_norm.dim() == 0:
            y_f1_norm = y_f1_norm.unsqueeze(0)
        if y_f2_norm.dim() == 0:
            y_f2_norm = y_f2_norm.unsqueeze(0)
        if m1x_f3_norm.dim() == 0:
            m1x_f3_norm = m1x_f3_norm.unsqueeze(0)
        if m1x_f4_norm.dim() == 0:
            m1x_f4_norm = m1x_f4_norm.unsqueeze(0)
        self.y_f1_norm = y_f1_norm
        self.y_f2_norm = y_f2_norm
        self.m1x_f3_norm = m1x_f3_norm
        self.m1x_f4_norm = m1x_f4_norm
    ...
    def KNet_step(self, y):
        ...
        # Innovation
        y_obs = y.unsqueeze(0).unsqueeze(0)
        self.m1y = self.m1y.unsqueeze(0).unsqueeze(0)
        dy = y_obs - self.m1y
    ...
    def init_hidden(self):
        ...
        # new head
        weight = next(self.parameters()).data
        hidden = weight.new(1, 1, self.new_head_hidden_gru).zero_()
    ...
    def build_q_head(self):
        ...
        self.act1 = nn.ReLU()
        self.hn1 = torch.randn(n_layers, 1, self.new_head_hidden_gru)
        # default GRU: expects (seq_len, batch, input_dim)
        self.gru = nn.GRU(input_size=self.new_head_hidden_size,
                          hidden_size=self.new_head_hidden_gru,
                          num_layers=n_layers)

        self.fc2 = nn.Linear(self.new_head_hidden_gru, self.new_head_hidden_size)
        self.act2 = nn.ReLU()
    ...
    def forward_q_head(self, x):
        ...
        input = torch.cat([x.view(-1), self.y_f1_norm,self.y_f2_norm, self.m1x_f3_norm,self.m1x_f4_norm ], dim=0)
        # ans = self.q_head(x)
