...
def UKFTest(SysModel, test_input, test_target, modelKnowledge='full', allStates=True, init_cond=None):
    ...
    UKF_cov = torch.empty([N_T, SysModel.m,SysModel.m, SysModel.T_test])
    start = time.time()
    for j in range(0, N_T):
        ...
        for z in range(0, SysModel.T_test):
            ...
            UKF_cov[j, :, :, z] = torch.from_numpy(UKF.P)
    ...
    return [MSE_UKF_linear_arr, MSE_UKF_linear_avg, MSE_UKF_dB_avg, UKF_out, UKF_cov]
