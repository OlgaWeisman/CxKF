...
def gaussian_nll(target, predicted_mean, predicted_var):
    ...
    sample_wise_error = torch.sum(element_wise_nll, dim=0)
    ...
    def NNTrain(self, n_Examples, train_input, train_target, n_CV, cv_input, cv_target):
        ...
        for ti in range(0, self.N_Epochs):
            ...
            for j in range(0, self.N_B):
                ...
                KF_cov = torch.empty(self.ssModel.m, self.ssModel.T, self.num_of_mc_iterations)
                ...
                for mc_idx in range(0, self.num_of_mc_iterations):
                    self.model.InitSequence(self.ssModel.m1x_0, self.ssModel.m2x_0)
                    for t in range(0, self.ssModel.T):
                        x_out_training[:, t, mc_idx],regularization[:, t, mc_idx],KF_cov[:, t, mc_idx] = self.model(y_training[:, t])
                cov_out_training = torch.var(x_out_training, dim=-1)
                # # Compute Training Loss
                #LOSS = gaussian_nll(train_target[n_e, :, :], torch.mean(x_out_training, dim = -1), cov_out_training)
            ...
            for j in range(0, self.N_CV):
                ...
                cov_out_cv = torch.empty( self.ssModel.m,self.ssModel.T)
                for mc_idx in range(0, self.num_of_mc_iterations):

                    self.model.InitSequence(self.ssModel.m1x_0,self.ssModel.m2x_0) # Initialize the cov

                    for t in range(0, self.ssModel.T):
                        x_out_cv[:, t, mc_idx], _, _ = self.model(y_cv[:, t])

                # cov_out_cv = torch.var(x_out_cv, dim=-1)
                # Compute Training Loss
        ...
        for ii in range(0, n_Trails):
            ...
            for j in range(0, num_of_calib):
                ...
                for mc_idx in range(0, self.num_of_mc_iterations):
                    self.model.InitSequence(self.ssModel.m1x_0, self.ssModel.m2x_0)
                    for t in range(0, self.ssModel.T):
                        x_out_test[:, t, mc_idx], _, _ = self.model(y_mdl_tst[:, t])
                # calculate q_low and q_hi as Guss
                cov_out_training = torch.var(x_out_test, dim=-1)
                mean_x_out = torch.mean(x_out_test, dim=-1)
                q_low_tmp, q_high_tmp, _ = compute_gaussian_quantiles(mean_x_out.detach().numpy(), cov_out_training.detach().numpy(), alpha)

                q_test[0, :] = torch.from_numpy(q_low_tmp).float()
                q_test[1, :] = torch.from_numpy(q_high_tmp).float()
            ...
            for j in range(0, self.N_T):
                ...
                # calculate q_low and q_hi as Guss
                cov_out_test = torch.var(x_out_test, dim=-1)
                mean_x_out = torch.mean(x_out_test, dim=-1)
                q_low_tmp, q_high_tmp, _ = compute_gaussian_quantiles(mean_x_out.detach().numpy(), cov_out_test.detach().numpy(), alpha)
