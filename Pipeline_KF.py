    ...
    def NNTrain(self, n_Examples, train_input, train_target, n_CV, cv_input, cv_target):
        ...
        for ti in range(0, self.N_Epochs):
            ...
            for j in range(0, self.N_CV):
                ...
                MSE_cv_linear_batch[j] = self.loss_fn(x_out_cv, cv_target[j, :, :self.ssModel.T]).item()
            ...
            for j in range(0, self.N_B):
                ...
                # Compute Training Loss
                Beta = 0.1 * (ti / self.N_Epochs)
                LOSS = (1-Beta)*self.loss_fn(x_out_training, train_target[n_e, :, :self.ssModel.T]) + Beta*pinball_loss(x_out_training, train_target[n_e, :, :self.ssModel.T], [0.05,0.95])
                # LOSS = gaussian_nll(train_target[n_e, :, :], x_out_training, cov_out_training)
