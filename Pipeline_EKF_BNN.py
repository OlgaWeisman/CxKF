        ...
        for ti in range(0, self.N_Epochs):
            ...
            for j in range(0, self.N_B):
                ...
                for mc_idx in range(0, self.num_of_mc_iterations):
                    ...
                    for t in range(0, self.ssModel.T):
                        x_out_training[:, t, mc_idx], regularization[:, t, mc_idx] = self.model(y_training[:, t])
                        if torch.isnan(x_out_training[0, t, mc_idx] ):
                            print("there is nan in t= ",t, "and mc_idx = ",mc_idx)
                cov_out_training = torch.var(x_out_training, dim=-1)
                # Compute Training Loss
                #LOSS = self.loss_fn(torch.mean(x_out_training, dim = -1), train_target[n_e, :, :])
            ...
            for j in range(0, self.N_CV):
                ...
                for mc_idx in range(0, self.num_of_mc_iterations):
                    ...
                    for t in range(0, self.ssModel.T):
                        x_out_cv[:, t, mc_idx], _ = self.model(y_cv[:, t])
    ...
    def NNTest(self, n_Test, test_input, test_target):
        ...
        for j in range(0, self.N_T):
            ...
            for mc_idx in range(0, self.num_of_mc_iterations):
                self.model.InitSequence(self.ssModel.m1x_0, self.ssModel.T)
                for t in range(0, self.ssModel.T):
                    x_out_test[:, t, mc_idx], _ = self.model(y_mdl_tst[:, t])
