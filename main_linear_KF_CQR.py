...
from Pipeline_KF_origin_CP import Pipeline_KF
...
### Design Model ###
####################
r2 = torch.tensor([10])#torch.tensor([10, 1., 1e-1])
...
for index in range(0, len(r2)):
    ...
    modelFolder = 'KNet' + '/'
    KFQuantile_Pipeline = Pipeline_KF(strTime, "KNet", "KNet_" + dataFileName[index])
    KFQuantile_Pipeline.setssModel(sys_model)
    KFQuantile_model = QuantileNN(new_head_in_shape=2)
    KFQuantile_Pipeline.setModel(KFQuantile_model)
    KFQuantile_Pipeline.setTrainingParams(n_Epochs=500, n_Batch=30, learningRate=1E-3, weightDecay=1E-5)

    # KNet_Pipeline.model = torch.load(modelFolder+"model_KNet.pt")

    KFQuantile_Pipeline.NNTrain(N_E, train_input, train_target, N_CV, cv_input, cv_target)


    KFQuantile_Pipeline.NNCalibrate_Predict(N_E, J_T, test_input, test_target, alpha)
