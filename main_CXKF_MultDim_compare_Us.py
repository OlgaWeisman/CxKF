...
# User configuration
# ======================================================
scenario = "linear"   # options: "linear", "nonlinear", "lorenz", "nonlonear-partial"
model_type = "KF"       # options: "KF", "EKF", "UKF"
coverage_mode = "sample" #"trajectory"
...
for index in range(len(r2)):
    ...
    # Storage
    # -----------------------------
    error_NPDQR_256 = torch.empty(T, Epochs_num)
    IW_NPQR_256 = torch.empty(T, Epochs_num)
    s_dqr_256 = torch.empty(Test_samples, T, Epochs_num)

    error_NPDQR_128 = torch.empty(T, Epochs_num)
    IW_NPQR_128 = torch.empty(T, Epochs_num)
    s_dqr_128 = torch.empty(Test_samples, T, Epochs_num)


    error_NPDQR_64 = torch.empty(T, Epochs_num)
    IW_NPQR_64 = torch.empty(T, Epochs_num)
    s_dqr_64 = torch.empty(Test_samples, T, Epochs_num)

    error_NPDQR_16 = torch.empty(T, Epochs_num)
    IW_NPQR_16 = torch.empty(T, Epochs_num)
    s_dqr_16 = torch.empty(Test_samples, T, Epochs_num)

    error_NPDQR_4 = torch.empty(T, Epochs_num)
    IW_NPQR_4 = torch.empty(T, Epochs_num)
    s_dqr_4 = torch.empty(Test_samples, T, Epochs_num)
    ...
    #     debug_plot=False,
    # )
    # dqr_4.u_list = make_rectangular_directions(dim)
    #
    # # -----------------------------
    # # No-learning elliptical model
    # # -----------------------------
    # dqr_elip = copy.deepcopy(dqr_128)
    # dqr_elip.num_u = None
    ...
    # # -----------------------------
    # dqr_rect = copy.deepcopy(dqr_128)
    ...
    # Train/load DQR
    # ======================================================
    dqr_256.fit_nonLinear_forall_seq(
            x_input_train,
            train_target,
            x_input_cv,
            cv_target,
    ...
    # dqr_128.fit_nonLinear_forall_seq(
    #         x_input_train,
    #         train_target,
    #         x_input_cv,
    #         cv_target,
    ...
    # )
    #
    # dqr_64.fit_nonLinear_forall_seq(
    #         x_input_train,
    #         train_target,
    #         x_input_cv,
    #         cv_target,
    ...
    # )
    #
    # dqr_16.fit_nonLinear_forall_seq(
    #         x_input_train,
    #         train_target,
    #         x_input_cv,
    #         cv_target,
    ...
    # )
    #
    # dqr_4.fit_nonLinear_forall_seq(
    #         x_input_train,
    #         train_target,
    #         x_input_cv,
    #         cv_target,
    ...
    for epochs in range(Epochs_num):
        ...
        for t in range(0, T):
            ...
            if coverage_mode == "sample":
                dqr_256.calibrate_crc(x_input_calib_per_t, target_x_calib[:, :, t], alpha)
                # dqr_128.calibrate_crc(x_input_calib_per_t, target_x_calib[:, :, t], alpha)
                # dqr_64.calibrate_crc(x_input_calib_per_t, target_x_calib[:, :, t], alpha)
                # dqr_16.calibrate_crc(x_input_calib_per_t, target_x_calib[:, :, t], alpha)
                # dqr_4.calibrate_crc(x_input_calib_per_t, target_x_calib[:, :, t], alpha)
            ...
            error_NPDQR_256[t, epochs], IW_NPQR_256[t, epochs],s_dqr_256[:, t, epochs] = dqr_256.inference_new(
                x_input_test_per_t, target_x_test[:, :, t], target_x_test[:, :, t], False
            )

            # error_NPDQR_128[t, epochs], IW_NPQR_128[t, epochs],s_dqr_128[:, t, epochs] = dqr_128.inference_new(
            #     x_input_test_per_t, target_x_test[:, :, t], target_x_test[:, :, t], False
            # )
            #
            # error_NPDQR_64[t, epochs], IW_NPQR_64[t, epochs],s_dqr_64[:, t, epochs] = dqr_64.inference_new(
            #     x_input_test_per_t, target_x_test[:, :, t], target_x_test[:, :, t], False
            # )
            #
            # error_NPDQR_16[t, epochs], IW_NPQR_16[t, epochs],s_dqr_16[:, t, epochs] = dqr_16.inference_new(
            #     x_input_test_per_t, target_x_test[:, :, t], target_x_test[:, :, t], False
            # )
            #
            # error_NPDQR_4[t, epochs], IW_NPQR_4[t, epochs],s_dqr_4[:, t, epochs] = dqr_4.inference_new(
            #     x_input_test_per_t, target_x_test[:, :, t], target_x_test[:, :, t], False
            # )
            #
            #
    ...
    writer.append_metrics(
        algo="DQR-CPKF-256",
        snr_db=snr_db,
        C=error_NPDQR_256.mean(dim=1),
        S=(s_dqr_256.sum(dim=1) > 0).float().mean(),
        WI=IW_NPQR_256.mean(dim=1),
    )
    # writer.append_metrics(
    #     algo="DQR-CPKF-128",
    #     snr_db=snr_db,
    #     C=error_NPDQR_128.mean(dim=1),
    #     S=(s_dqr_128.sum(dim=1) > 0).float().mean(),
    #     WI=IW_NPQR_128.mean(dim=1),
    # )
    #
    # writer.append_metrics(
    #     algo="DQR-CPKF-64",
    #     snr_db=snr_db,
    #     C=error_NPDQR_64.mean(dim=1),
    #     S=(s_dqr_64.sum(dim=1) > 0).float().mean(),
    #     WI=IW_NPQR_64.mean(dim=1),
    # )
    #
    # writer.append_metrics(
    #     algo="DQR-CPKF-16",
    #     snr_db=snr_db,
    #     C=error_NPDQR_16.mean(dim=1),
    #     S=(s_dqr_16.sum(dim=1) > 0).float().mean(),
    #     WI=IW_NPQR_16.mean(dim=1),
    # )
    #
    # writer.append_metrics(
    #     algo="DQR-CPKF-4",
    #     snr_db=snr_db,
    #     C=error_NPDQR_4.mean(dim=1),
    #     S=(s_dqr_4.sum(dim=1) > 0).float().mean(),
    #     WI=IW_NPQR_4.mean(dim=1),
    # )
    #
    # writer.append_metrics(
