...
from Pipeline_EKF_CP import Pipeline_EKF
from Extended_KalmanNet_arch1_GRU import KalmanNetNN
...
for index in range(0, len(r2)):
    ...
    dataFileName = [ '1x1_rq020_nonLinear.pt', '1x1_rq1030_nonLinear.pt']
    ...
    # DataGen(sys_model_partialf, dataFolderName + dataFileName[index], T, T_test,randomInit=False)
    # print("Data Load")
    # [train_input_partialf, train_target_partialf, cv_input_partialf, cv_target_partialf, test_input_partialf, test_target_partialf] = DataLoader(
    #     dataFolderName + dataFileName[index])
    ...
    modelFolder = 'KNet' + '/'
    KNet_Pipeline = Pipeline_EKF(strTime, "KNet", "KNet_" + dataFileName[index])
    KNet_Pipeline.setssModel(sys_model_partialf)
    KNet_model = KalmanNetNN()
    KNet_model.Build(sys_model_partialf)
    KNet_Pipeline.setModel(KNet_model)
    KNet_Pipeline.setTrainingParams(n_Epochs=500, n_Batch=50, learningRate=1e-3, weightDecay=1E-5)
    KNet_Pipeline.NNTrain(N_E, train_input, train_target, N_CV, cv_input, cv_target)


    KNet_Pipeline.NNCalibrate_Predict(N_E, J_T, test_input, test_target, alpha)

    # # Calculate loss
    # KNet_Pipeline.NNTest(N_T, test_input, test_target)
    # KNet_Pipeline.save()
    # [MSE_EKF_linear_arr, MSE_EKF_linear_avg, MSE_EKF_dB_avg, KG_array, EKF_out] = EKFTest(sys_model_partialf, test_input, test_target)
