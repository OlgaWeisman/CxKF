...
for index in range(0, len(r2)):
    ...
    sys_model = SystemModel(f, q, h, r, T, T_test, m, n, "Lorenz_Atractor", device = dev)
    ...
    ###################################
    dataFolderName = 'Simulations/Lorenz_Atractor' + '/'
    ...
    with wandb.init(
        ...
        if not train_model: load_path = "dqr_model.pt"
    ...
    # ) as run:
    #
    #     if not train_model: load_path = "dqr_fixing_model.pt"
    ...
    if not train_model: load_path = "dqr_small_model.pt"
