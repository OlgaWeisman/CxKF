    ...
    def NNTrain(self, n_Examples, train_input, train_target, n_CV, cv_input, cv_target):
        ...
        for ti in range(0, self.N_Epochs):
            ...
            for j in range(0, self.N_B):
                ...
                self.model.InitSequence(self.ssModel.m1x_0.repeat(1,self.num_of_mc_iterations)) # repmat for MC iterations
                ...
                for t in range(0, self.ssModel.T):
                    ...
                    cov_numer = torch.zeros(self.ssModel.m, self.num_of_mc_iterations)
                    for mc_idx in range(0, self.num_of_mc_iterations):
                        x_out_training[:, t, mc_idx], regularization[:, t, mc_idx], cov_numer[:, mc_idx] = self.model(y_training[:, t], cov_out_training_tmp)
                    ...
                    cov_out_training_tmp = torch.mean(cov_numer .detach(), dim=-1)
                    cov_out_training_list.append(cov_out_training_tmp, )
            ...
            for j in range(0, self.N_CV):
                ...
                for t in range(0, self.ssModel.T):
                    ...
                    for mc_idx in range(0, self.num_of_mc_iterations):
                        x_out_cv[:, t, mc_idx], _, cov_numer[:, mc_idx] = self.model(y_cv[:, t], cov_out_cv_tmp)

                    cov_out_cv_tmp = torch.mean(cov_numer, dim=-1)#torch.var(x_out_cv[:, t, :].clone(), dim=-1)  # or True if needed
    ...
    def NNTest(self, n_Test, test_input, test_target):
        ...
        for j in range(0, self.N_T):
            ...
            for t in range(0, self.ssModel.T):
                ...
                for mc_idx in range(0, self.num_of_mc_iterations):
                    x_out_test[:, t, mc_idx], _ = self.model(y_mdl_tst[:, t], cov_out_test_tmp)
