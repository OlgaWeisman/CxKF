    ...
    def NNTrain(self, n_Examples, train_input, train_target, n_CV, cv_input, cv_target):
        ...
        for ti in range(0, self.N_Epochs):
            ...
            for j in range(0, self.N_B):
                ...
                KF_cov = torch.zeros(self.ssModel.m, self.ssModel.T, self.num_of_mc_iterations)
                ...
                for mc_idx in range(0, self.num_of_mc_iterations):
                    ...
                    for t in range(0, self.ssModel.T):
                        x_out_training[:, t, mc_idx],regularization[:, t, mc_idx],KF_cov[:, t, mc_idx] = self.model(y_training[:, t],KF_cov[:, t-1, mc_idx].detach())
                cov_out_training = torch.mean(KF_cov.detach(),dim=-1)#torch.var(x_out_training, dim=-1)
            ...
            for j in range(0, self.N_CV):
                ...
                for mc_idx in range(0, self.num_of_mc_iterations):
                    ...
                    for t in range(0, self.ssModel.T):
                        x_out_cv[:, t, mc_idx], _, KF_cov[:, t, mc_idx] = self.model(y_cv[:, t], KF_cov[:, t-1, mc_idx])
    ...
    def NNTest(self, n_Test, test_input, test_target):
        ...
        for j in range(0, self.N_T):
            ...
            for mc_idx in range(0, self.num_of_mc_iterations):
                self.model.InitSequence(self.ssModel.m1x_0,self.ssModel.m2x_0)
                for t in range(0, self.ssModel.T):
                    x_out_test[:, t, mc_idx], _, KF_cov[:, t, mc_idx] = self.model(y_mdl_tst[:, t],KF_cov[:, t-1, mc_idx])
