...
class Pipeline_EKF:
    ...
    def NNTrain(self, n_Examples, train_input, train_target, n_CV, cv_input, cv_target):
        ...
        Q_W_batch = torch.empty([self.N_B])
        self.Q_W_epoch = torch.empty([self.N_Epochs])
        ...
        for ti in range(0, self.N_Epochs):
            ...
            x_out_cv_all, cov_out_cv_all = EKFTest(self.ssModel, cv_input)
            for j in range(0, self.N_CV):
                q_cv = torch.empty(2, self.ssModel.T)
                x_out_cv = x_out_cv_all[j, :]
                cov_out_cv = cov_out_cv_all[j, :]
                for t in range(0, self.ssModel.T):
                    q_cv[:, t] = self.model(x_out_cv[t],cov_out_cv[t])
            ...
            x_out_training_all, cov_out_training_all = EKFTest(self.ssModel, train_input)
            for j in range(0, self.N_B):
                n_e = random.randint(0, self.N_E - 1)

                x_out_training = x_out_training_all[n_e, :]
                q_train = torch.empty(2, self.ssModel.T) #TODO
                cov_out_training = cov_out_training_all[n_e, :]

                for t in range(0, self.ssModel.T):
                    q_train[:, t] = self.model(x_out_training[t], cov_out_training[t])

                # Compute Training Loss
                LOSS_PD = pinball_loss(train_target[n_e, :, :self.ssModel.T], q_train, [0.05, 0.95])
                LOSS = LOSS_PD
                Q_W = torch.diff(q_train[:, 0], dim=0)
                Q_W_batch[j] = Q_W.item()
            ...
            self.Q_W_epoch[ti] = torch.mean(Q_W_batch)
            ##################
            ### Optimizing ###
            ##################
            ...
            print(ti, "MSE Training :", self.MSE_train_dB_epoch[ti], "[dB]", "MSE Validation :",
                  ...
                  "[dB]")
            print(ti, "Q width Training :", self.Q_W_epoch[ti])
    ...
    def NNCalibrate_Predict(self, n_Calib, n_Trails, calib_input, calib_target, alpha):
        ...
        # for CQKF
        [x_array_train, sigma_array_train] = EKFTest(self.ssModel, calib_input)
        [q_low, q_hi, q_half] = compute_gaussian_quantiles(x_array_train.cpu(), sigma_array_train.cpu(), alpha)
        for ii in range(0, n_Trails):
            ...
            x_out_test_all, cov_out_test_all = EKFTest(self.ssModel, calibration_input)
            for j in range(0, num_of_calib):
                x_out_test = x_out_test_all[j, :]
                cov_out_test = cov_out_test_all[j, :]
                q_test = torch.empty(2, self.ssModel.T)
                for t in range(0, self.ssModel.T):
                    q_test[:, t] = self.model(x_out_test[t], cov_out_test[t])
            ...
            x_out_test_all, cov_out_test_all = EKFTest(self.ssModel, test_input)
            for j in range(0, self.N_T):
                ...
                #
                #     plt.plot(x_out_test.detach().cpu().squeeze(), "ro")
                #
                #     plt.fill_between(
